    ...
    <v-card>
      ...
      <v-form ref="passwordResetForm" v-model="passwordResetValid" @submit.prevent="passwordResetSubmit">
        <v-card-text class="text-center">
          <v-row>
            <v-col cols="12">
              <v-alert v-if="error" type="error" dense dismissible> Password reset link is invalid or has expired. </v-alert>
        ...
        <v-card-actions class="justify-center">
          <v-btn v-if="!requestSent" class="hvr-shrink pl-5 pr-5 bg-primary mb-2" size="x-large" rounded type="submit"> Reset Password </v-btn>
        </v-card-actions>
      </v-form>
